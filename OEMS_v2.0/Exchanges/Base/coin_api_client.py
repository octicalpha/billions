...
import gzip
import json
PRODUCTION_URL = 'https://rest-us-east-altcoinadvisors.coinapi.io/v1%s'

class HTTPClient:
    def __init__(self, productionUrl, endpoint, headers = dict(), params = dict()):
        self.productionUrl = productionUrl
        self.url = productionUrl % endpoint
        self.params = params
    ...
    def perform(self):
        ...
        request = Request(resource, headers=self.headers)
...
class CoinAPIv1:
    ...
    def __init__(self, api_key, productionUrl, headers = dict(), client_class=HTTPClient):
        ...
        self.headers.update(header_apikey)
    ...
    def with_header(self, header, value):
        ...
        params = {}
        params.update(old_headers)
        params.update(new_header)
        return CoinAPIv1(self.api_key, params)

    def with_headers(self, additional_headers):
        old_headers = self.headers
        params = {}
        params.update(old_headers)
        params.update(additional_headers)
        return CoinAPIv1(self.api_key, params)

    def metadata_list_exchanges(self):
        request = MetadataListExchangesRequest()
        client = self.client_class(self.productionUrl, request.endpoint(), self.headers)
    ...
    def metadata_list_symbols(self, query_parameters):
        request = MetadataListSymbolsRequest()
        client = self.client_class(self.productionUrl, request.endpoint(), self.headers, query_parameters)
        return client.perform()
    ...
    def exchange_rates_get_specific_rate(self,
        ...
        request = ExchangeRatesGetSpecificRateRequest(asset_id_base,
                                                      asset_id_quote,
                                                      query_parameters)
        client = self.client_class(self.productionUrl, request.endpoint(),
                                   self.headers,
    ...
    def quotes_current_data_all(self, query_parameters):
